...
const trackStream = (stream2, chunkSize, onProgress, onFinish, encode2) => {
...
const configPath = path$1.join(__dirname, "../../src/main/rss-config.json");
...
async function getFeeds() {
  try {
    const data = await fs$1.readFile(configPath, "utf8");
      ...
      config.categories.map(async (category) => {
        ...
        return Promise.all(
          category.feeds.map(async (feed) => {
            try {
              const response = await axios$1.get(feed.url);
...
electron.app.whenReady().then(() => {
  ...
  electron.ipcMain.handle("get-feeds", async () => {
    try {
  ...
  electron.ipcMain.handle("open-external-url", (event, url2) => {
    if (url2) {
